...
(ratio2<-itr.per.sec [1]/itr.per.sec [2])
```
 - The computing of correlation does not perform as well as covariance. We can see from results that when applying `modified_cor()` on matrices, the computing speed of `modified_cor()` is slower than `cor()`. In test1, the ratio of speed is about `r round(ratio, digits = 2)` , while in test2, the ratio is about `r round(ratio2, digits = 2)`.
